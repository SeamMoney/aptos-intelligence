      ...
      <div className="w-[380px] min-w-[380px] border-r border-white/5 flex flex-col">
        ...
        <div className="p-4 border-b border-white/5 space-y-3">
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-emerald-400/40"
          />
        ...
        <div className="p-4 border-t border-white/5 flex items-center gap-6 text-xs text-white/40">
          <div><span className="text-white/60 font-medium">{reports.length}</span> reports</div>
          <div>avg <span className="text-white/60 font-medium">{avgImportance}</span>/10</div>
          <div><span className="text-white/60 font-medium">{features.length}</span> features</div>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {!activeReport ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-white/20">
              <p className="text-2xl font-light">Select a report to begin</p>
            ...
            <div className="p-8 max-w-4xl mx-auto space-y-6">
              ...
              {/* Report content */}
              <div className="prose prose-invert max-w-none prose-headings:text-white/90 prose-p:text-white/70 prose-li:text-white/70 prose-code:text-emerald-400 prose-pre:bg-white/5 prose-pre:border prose-pre:border-white/10">
                {activeTab === "advanced" ? (
              ...
              {/* Importance badge */}
              <div className="border-t border-white/5 pt-4 flex items-center gap-4 text-sm text-white/40">
                <span>Importance: <span className={`font-mono font-medium ${activeReport.importance >= 8 ? "text-red-400" : activeReport.importance >= 6 ? "text-amber-400" : "text-white/60"}`}>{activeReport.importance}/10</span></span>
                <span className="text-white/20">·</span>
