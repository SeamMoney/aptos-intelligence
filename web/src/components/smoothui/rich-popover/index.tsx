...
export default function RichTooltip({
  ...
  return (
    <PopoverRoot>
      ...
      <PopoverPortal>
        <PopoverContent align={align} className={`z-50 ${className}`} side={side} sideOffset={8} render={<motion.div animate={
                            shouldReduceMotion
                              ? { opacity: 1 }
                              : { opacity: 1, scale: 1, y: 0, filter: "blur(0px)" }
                          } className="relative rounded-2xl border border-white/10 bg-black px-4 py-3 text-white shadow-xl" exit={
                            shouldReduceMotion
                          ...
                          )}{/* Tail */}<PopoverArrow className="fill-black" /></PopoverContent>
      </PopoverPortal>
    </PopoverRoot>
  );
}
